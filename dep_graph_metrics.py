...
class SuffixTree:
    ...
    def deepestInternalNode(self):
        if self.children == []:
            return []
        
        subresults = map(lambda c: c.deepestInternalNode(), self.children)
        
        m = max(subresults, key = len)
        return self.instructions + m
...
def processFile(tupleArg):
    ...
    try:
        ...
        toWrite+=f"File: {fileName}\n"
        toWrite+=f"\tNumber of subcircuits: {len(ast.subcircuits)}\n\n"
        ...
        for index, subcircuit in enumerate(ast.subcircuits):
            ...
            toWrite+=f"\tLongest repeating gates in subcircuit {index} ({len(res)} gates in the repeated segment for a total of {len(subcircuit.instructions)} gates in the subcircuit):\n\t{res if res != [] else '<None>'}\n"
        toWrite+="\n\n"
        writeToFile(toWrite)
    except:
        print(f"File {fileName} gave error")
